...
function makeFileList(a,c){c=c?"<br />":" ";let b="",d;a=a.split("\x00");let e,f,l,h,g=encodeURIComponent(curDir).length;for(d in a)for(l in e=a[d].split("&"),f="",e)h=e[l].split("="),0===1*l?f=h[1].substr(g):b+=f+"%2F"+h[1]+c;return b.replace(/%2F/g,"/")}function doesSupportAjaxUploadWithProgress(){var a;if(a="undefined"!==typeof window.FileList)a=new XMLHttpRequest,a=!!(a&&"upload"in a&&"onprogress"in a.upload);return a}
...
function fils2up(){if(""===document.cliterm.cmdlin.value)return alert("Please enter a command."),!1;let a=document.cliterm.cmdlin.value;0<a.indexOf("$$")&&(document.cliterm.cmdlin.value=a.replace("$$",makeFileList(sessionStorage.fmx_mrkd,!1)));document.cliterm.submit();return!0}
function selectionAction(a){var c="";"undefined"!==window.getSelection?c=window.getSelection():"undefined"!==document.selection&&"Text"==document.selection.type&&(c=document.selection.createRange().text);c=curDir+c;a?doEditFile(c):doViewFile(c)}function display_cmmStorage(a){if(a){var c="";a=a.split("\x00");for(let b in a){let d=decodeURIComponent(a[b]).split("&");for(let e=0;e<d.length;e++)0<e&&(c+="    "),c+=d[e]+"\n"}alert(c)}else alert("[ empty ]")}
...
function cm_dup(a){a={act:"dupl",fref:curDir+a.closest("[data-fref]").dataset.fref};postAndRefresh(a)}function cm_ren(a){a=a.closest("[data-fref]").dataset.fref;doRenameFile(a,event)}function cm_zip(a,c){a=a.closest("[data-fref]").dataset.fref;let b=document.forms.cliterm,d="zip ";c&&(d+="-r ");c=a.replace(/\s/g,"_");b.cmdlin.value=d+c+'.zip "'+a+'"';b.submit()}
