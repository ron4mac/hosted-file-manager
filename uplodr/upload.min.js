...
e.maxfilesize&&(w="File is larger than allowed");if(w)a.pBar.msg(w,!0),C(b.size),a.X=null,v(!0,"errM");else return a.X.upload.onload=l.load,a.X.upload.onerror=l.fail,a.X.upload.onabort=l.abrt,a.X.upload.onprogress=l.prog,a.doChnk?k():g(),a}var e=Object.assign({},{lodrdiv:"uplodr",upURL:"upload.php",payload:{},maxFilesize:134217728,dropMessage:"Drop files here to upload<br>(or click to select)",concurrent:3,maxchunksize:16777216,allowed_file_types:[],doneFunc:null},z),E,B,x,h=[],y=e.concurrent,D=e.allowed_file_types,
M=e.maxchunksize,q=!1,r=0,F=0,G=0,H=0,I=0,A=0,t,u,N="";let n=(b,a,c)=>{b=document.createElement(b);a&&(b.innerHTML=a);for(let g in c)b.setAttribute(g,c[g]);return b},J=b=>{b.stopPropagation();b.preventDefault();b.target.className="dragover"==b.type?"hover":"dropzone"},K=b=>{let a,c;b instanceof FileList||(J(b),b=b.target.files||b.dataTransfer.files);H&&(H=F=G=0);for(a=0;c=b[a];a++)F+=c.size,h.push(c),x.innerHTML=h.length,v(!1,"fsel");h.length>y&&(t.style.display="inline-block")},O=()=>{if(!q){H=1;
if("function"==typeof e.doneFunc){let b=I,a=A;setTimeout(function(){e.doneFunc(b,a)},500)}A=I=0}},v=(b,a)=>{b&&("ufo"==a&&I++,--r||O());!q&&h.length&&(!y||r<y)&&(b=h.shift(),new Q(b),r++,x.innerHTML=h.length);0>=h.length&&(t.style.display="none",u.style.display="none")},C=b=>{!E||0>b||(G+=b,E.style.width=100*G/F+"%")},m=(b,a)=>{for(let c in a)b.append(c,a[c])};f.H5uSetup=()=>{var b=$id(e.lodrdiv);f.File&&f.FileList?(b.appendChild(n("div",'<input type="file" name="userpictures" id="file_field" multiple="multiple" accept="image/*,video/*" style="display:none"><div class="drpmsg">'+
