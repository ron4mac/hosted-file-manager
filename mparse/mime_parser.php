...
*/
	Function Analyze($message, &$results)
	{
		...
		elseif (is_array($message['Headers']['content-type:'])) {
			if (!$this->SetPositionedWarning('message contains multiple content-type headers', $message['Position'])) return(0);
...
{
	include 'mparse/rfc822_addresses.php';

	$mpo = new mime_parser_class();
	//$mpo->ParseFile($ffref);
	//'<xmp>'.print_r($mpo, true).'</xmp>';
	$parameters=array('File'=>$fref);
 ...
 */
	...
	if(defined('__TEST'))
	{
		...
		if(IsSet($__test_options['mbox']))
			$mpo->mbox=$__test_options['mbox'];
		if(IsSet($__test_options['decode_bodies']))
			$mpo->decode_bodies=$__test_options['decode_bodies'];
		if(IsSet($__test_options['use_part_file_names']))
			$mpo->use_part_file_names=$__test_options['use_part_file_names'];
	}

	if(!$mpo->Decode($parameters, $decoded))
	{
		echo 'MIME message decoding error: '.$mpo->error.' at position '.$mpo->error_position;
		if($mpo->track_lines
		&& $mpo->GetPositionLine($mpo->error_position, $line, $column))
	...
	//		echo 'Message ',($message+1),':',"\n";
		//	echo'<xmp>'.$message;print_r($decoded[$message]);echo'</xmp>';
			if($mpo->decode_bodies)
			{
				if($mpo->Analyze($decoded[$message], $results)) {
					echo'<xmp>';print_r($results);echo'</xmp>';
				    $F = $T = $R = $M = $S = $D = '';
				    $F = my_addr_extr($results['From']);
				    $T = my_addr_extr($results['To']);
				    if (isset($results['Reply-to'])) $R = my_addr_extr($results['Reply-to']);
				    $S = $results['Subject'];
				    $D = $results['Date'];
				//    $M = strip_tags($results['Data'], '<br><p><div><a>');
				    if ($results['Type']=='html') {
				        $M = $results['Data'];
				    } else {
				        $M = nl2br($results['Data']);
				    }
				    echo 'Date: '.$D.'<br>From: '.$F.'<br>To: '.$T.'<br>Subject: '.$S.'<br>Reply: '.$R.'<br><br>'.$M.'<br><hr>';
				    }
				else
					echo 'MIME message analyse error: '.$mpo->error."\n";
			}
		}
		for($warning = 0, Reset($mpo->warnings); $warning < count($mpo->warnings); Next($mpo->warnings), $warning++)
		{
			$w = Key($mpo->warnings);
			echo 'Warning: ', $mpo->warnings[$w], ' at position ', $w;
			if($mpo->track_lines
			&& $mpo->GetPositionLine($w, $line, $column))
				echo ' line '.$line.' column '.$column;
			echo "\n";
		}
	}
