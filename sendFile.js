var sendFile = 0;	//1024000; // maximum allowed file size
...
sendFile = (function(toString, maxSize) {
	...
	return function(handler) {
		if (maxSize && maxSize < handler.file.fileSize) {
			if (isFunction(handler.onerror)) handler.onerror();
			return;
		}
		...
		xhr.onreadystatechange = function() {
				if (xhr.readyState != 4) {
					stateBuff.push(xhr.readyState,xhr.status);
					return;
				}
				//alert(stateBuff+' | '+xhr.readyState+' : '+xhr.status);
			};
		xhr.open("post", (handler.url || "?upload=true") + handler.location, true);
