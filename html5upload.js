(function($) {
	jQuery.fn.html5_upload = function(inopts) {
		...
		var options = jQuery.extend({
			...
			onStartOne: function(event, name, number, total) { return true; },
			...
			setTotalProgress: function(value) { },

			genName: function(file, number, total) { return file + " (" + (number+1) + " of " + total + ")"; },
			genStatus: function(progress, finished) {
				if (finished) { return 'FINISHED'; }
				if (progress === 0) { return 'STARTED'; }
				else if (progress == 1) { return 'LOADED'; }
				else { return 'PROGRESS'; }
				},
			genProgress: function(loaded, total) { return loaded / total; },
			genTotalProgress: function(loaded, total) { return loaded / total; }
			}, inopts);
		...
		function upload() {
			...
			this.disabled = true;
			var uploaded = 0;
			while (uploaded < total) { console.log(files[uploaded]);
				totalsiz += files[uploaded++].size; }
			uploaded = 0;
			options.setTotalProgress(options.genTotalProgress(0, totalsiz));
			...
			function upload_file(number) {
				...
				if (!$this.triggerHandler('html5_upload.onStartOne', [file.name, number, total])) {
				...
				xhr.upload.onprogress = function(rpe) {
					$this.trigger('html5_upload.onProgress', [rpe.loaded / rpe.total, file.name, number, total]);
					options.setStatus(options.genStatus(rpe.loaded / rpe.total));
					options.setProgress(options.genProgress(rpe.loaded, rpe.total));
					options.setTotalProgress(options.genTotalProgress(totalload+rpe.loaded, totalsiz));
				};
				xhr.onload = function(load) {
					$this.trigger('html5_upload.onFinishOne', [xhr.responseText, file.name, number, total]);
					options.setStatus(options.genStatus(1, true));
					options.setProgress(options.genProgress(file.size, file.size));
					options.setTotalProgress(options.genTotalProgress(totalload+file.size, totalsiz));
					totalload += file.size;
				...
				xhr.onerror = function(e) {
					$this.trigger('html5_upload.onError', [file.name, e]);
				...
				if (!options.sendBoundary) {
					...
					if (window.FormData) {//Many thanks to scottt.tw
						var f = new FormData(); //alert('fome');
						var fldNam = typeof(options.fieldName) == "function" ? options.fieldName() : options.fieldName;
						f.append(fldNam, file);
						f.append('fpath',filesPath);
						xhr.send(f);
					}
						...
						builder += 'Content-Disposition: form-data; name="fpath"';
						builder += crlf;
						builder += crlf;
						builder += filesPath;
						builder += crlf;
